   ...
   /*统计边数k*/ 
     for(subindex=0;subindex<lenght;subindex++)
    {
        ...
        //end
         ...
         // intre arc, soft smoothness constraint. revised by SJNiu 5/4/2014
         // x-direction
             if (x-qx_neg >= 0)
            {
                if (z+mx[x]>=0 && z+mx[x] < height && x+qx_pos < width)
                {
                    k1 = k1+1;
                }
            }
            ...
            // y-direction
             if (y-qy_neg >= 0)
            {
                if (z+my[x]>=0 && z+my[x] < height && y+qy_pos < dim)
                {
                    k1 = k1+1;
                }
            }
    ...
    for(subindex=0;subindex<lenght;subindex++)
    {
         ...
         // x-direction
            if (x-qx_neg >= 0)
            {
                subindex = z + (x-qx_neg)*height + y*width*height;
                if (z+mx[x]>=0 && z+mx[x] < height && x+qx_pos < width)
                {
                    ysubindex3 = z+mx[x] + (x+qx_pos)*height + y*width*height;
                    g->add_edge(ysubindex3,subindex,wx,wx);
                    //g->add_edge(subindex,ysubindex3,INF,wx)
                }
            }
            // y-direction
             if (y-qy_neg >= 0)
            {
                subindex = z + x*height + (y-qy_neg)*width*height;
                if (z+my[x]>=0 && z+my[x] < height && y+qy_pos < dim)
                {
                    ysubindex3 = z+my[x] + x*height + y+qy_pos*width*height;
                    g->add_edge(ysubindex3,subindex,wy,wy);
                    //g->add_edge(subindex,ysubindex3,INF,wy)
                }
            }
