function [bwimg, wimg, support ] = iat_pixel_warping( img, dx, dy,img2)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
wimg = zeros(size(img));
img  = double(img);
bwimg = zeros(size(img2));
% Interpolate pixel values to the new positions
for i=1:zim
    for j=1:rim
        for k=1:cim
            if XX(j,k,i)<1 | XX(j,k,i)>cim | YY(j,k,i)<1 | YY(j,k,i)>rim
                wimg(j,k,i) = 0;
                bwimg(j,k,i)=0;
            else
                wimg(j,k,i) = img(YY(j,k,i),XX(j,k,i),i);
                bwimg(j,k,i) = img2(YY(j,k,i),XX(j,k,i),i);
            end
        end
    end
