%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% aniso bilateral filtering for oct retinal image denoising     %%%%
%%%% improved by Qiang Chen on Jan 3, 2011, in Stanford University %%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% w     = [3,7];       % bilateral filter half-width
% sigma = [3 0.1]; % bilateral filter standard deviations

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Pre-process input and select appropriate filter.
function B = bfilter2_aniso(A,w,sigma)
...
for i = 1:dim(1)
   for j = 1:dim(2)
         ...
         iMin = max(i-w(1),1);
         iMax = min(i+w(1),dim(1));
         jMin = max(j-w(2),1);
         jMax = min(j+w(2),dim(2));
         ...
         F = H.*G((iMin:iMax)-i+w(1)+1,(jMin:jMax)-j+w(2)+1);
         B(i,j) = sum(F(:).*I(:))/sum(F(:));
